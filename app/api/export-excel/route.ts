...
function createWorksheet(
  ...
  if (data.length > 0) {
    ...
    const totalNilaiPerjanjian = data.reduce((sum, c) => sum + (c.nilaiPerjanjian || c.nilaiKontrak || 0), 0);
    const totalNilaiTagihan = data.reduce((sum, c) => sum + (c.nilaiTagihan || 0), 0);
    const totalTerbayar = data.reduce((sum, c) => sum + (c.terbayar || c.totalTagihanDibayar || 0), 0);

    // Find column indices for summary
    const nilaiPerjanjianColIdx = columns.findIndex((c) => c.key === "nilaiPerjanjian") + 1;
    const nilaiTagihanColIdx = columns.findIndex((c) => c.key === "nilaiTagihan") + 1;
    const terbayarColIdx = columns.findIndex((c) => c.key === "terbayar") + 1;

    // Merge cells for "TOTAL" label - use first few columns
    const labelEndCol = Math.max(nilaiPerjanjianColIdx - 1, 4);
    worksheet.mergeCells(rowNumber, 1, rowNumber, labelEndCol);

    const totalLabelCell = summaryRow.getCell(1);
    totalLabelCell.value = `TOTAL (${data.length} Kontrak)`;
    totalLabelCell.font = { bold: true, size: 11 };
    totalLabelCell.alignment = { horizontal: "right", vertical: "middle" };
    totalLabelCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE5E7EB" } };
    totalLabelCell.border = BORDER_STYLE;

    // Total Nilai Perjanjian
    if (nilaiPerjanjianColIdx > 0) {
      const cell = summaryRow.getCell(nilaiPerjanjianColIdx);
      cell.value = totalNilaiPerjanjian;
      cell.numFmt = '"Rp "#,##0';
      cell.font = { bold: true };
    ...
    // Total Nilai Tagihan
    if (nilaiTagihanColIdx > 0) {
      const cell = summaryRow.getCell(nilaiTagihanColIdx);
      cell.value = totalNilaiTagihan;
      cell.numFmt = '"Rp "#,##0';
      cell.font = { bold: true };
      cell.alignment = { horizontal: "right", vertical: "middle" };
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE5E7EB" } };
    ...
    // Total Terbayar
    if (terbayarColIdx > 0) {
      const cell = summaryRow.getCell(terbayarColIdx);
      cell.value = totalTerbayar;
      cell.numFmt = '"Rp "#,##0';
      cell.font = { bold: true };
      cell.alignment = { horizontal: "right", vertical: "middle" };
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE5E7EB" } };
    ...
    for (let i = labelEndCol + 1; i <= columns.length; i++) {
